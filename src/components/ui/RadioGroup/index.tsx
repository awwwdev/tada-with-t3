...
export default function RadioGroup({
  ...
  return (
    <RadioGroupContext.Provider value={{ value, setValue, name }}>
      <fieldset>
        <Legend preStyled className="text-sm c-base11 mt-0.5em font-display ">
          {legend}
        </Legend>
        <div className={`mt-1 flex gap-0.5em flex-wrap text-sm ${className}`}>{children}</div>
      </fieldset>
...
function RadioItem({
  ...
  const { value: controledValue, setValue, name } = useContext(RadioGroupContext);
  ...
  return (
    ...
    >
      <span>{label}</span>
      <div className="grid items-center align-content-center ">{children}</div>
      <input
        type="radio"
        name={name}
        value={value}
